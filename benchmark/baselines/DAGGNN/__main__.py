"""
DAG-GNN training script (CPU-only version).
Launched via HPC job array using 6 CLI arguments:
    1. Number of nodes (e.g., 20)
    2. Average degree (e.g., 3)
    3. Sample size(s) (comma-separated if multiple)
    4. Replicate ID
    5. Dataset type index (0 = linear, 1 = non-linear)
    6. Input scratch path

Saves a binarized adjacency matrix in results/{linear_gaussian|non_linear}/DAGGNN/...
"""
import argparse
import datetime
import math
...
# ================================================
# get data: experiments = {synthetic SEM}
# ================================================
# train_loader, valid_loader, test_loader, ground_truth_G = load_data( args, CONFIG.batch_size, CONFIG.suffix)
# --------------------------------------------
# Parse command-line arguments (PBS array mode)
# --------------------------------------------
x = sys.argv[1]             # n_nodes (e.g., "50")
dg = sys.argv[2]            # avg_degree (e.g., "3")
n_values = sys.argv[3]       # sample_size (e.g., "5000")
latent_levels = ["0L", "10L", "20L"]  # latent variable settings (0L, 10L, 20L)
idx_rep = int(sys.argv[4]) # replicate ID (int)
pt_input = sys.argv[5]      # input scratch path (e.g., $TMPDIR)
path_results_root = sys.argv[6]

# --------------------------------------------
# Handle sample size
# --------------------------------------------
if "," in n_values:
    sample_sizes = [s.strip() for s in n_values.split(",")]
else:
    sample_sizes = [n_values]

# --------------------------------------------
# Loop over all sample sizes (if vector)
# --------------------------------------------
for n in sample_sizes:
    # Loop over latent levels
    
    for lv in latent_levels:
        # path_file = os.path.join(pt_input, f"N{x}", dg, n, f"rep_{idx_rep}", f"input_{lv}.csv")
        path_file = os.path.join(pt_input, f"N{x}", dg, n, f"input_{lv}_{idx_rep}.csv")
        print("Input dataset path:", path_file)

        path_adj = os.path.join(path_results_root, f"N{x}", dg, n, f"adj_DAGGNN_{lv}_{idx_rep}.csv")
        os.makedirs(os.path.dirname(path_adj), exist_ok=True)
        ...
        if n_int <= 100:
            ...
            CONFIG.lr = 5e-05
            CONFIG.k_max_iter = 20

        CONFIG.cuda = False  # CPU only
        CONFIG.factor = not CONFIG.no_factor

        torch.manual_seed(CONFIG.seed)

        train_loader, CONFIG = load_data(CONFIG, path_file, CONFIG.batch_size)
        ...
        final_df.to_csv(path_adj, index=False, header=False)
          
        # Clean up input file
        # if os.path.exists(path_file):
        #    os.remove(path_file)
        # print(f"Saved adjacency matrix to {path_adj} and removed input file.")
