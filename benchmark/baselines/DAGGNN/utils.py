...
def load_data(config, path_file, batch_size=1000, debug=False):

    #print(path_file)
    # load dataset as numpy array
    X = np.expand_dims(
        np.loadtxt(
            os.path.expanduser(path_file),
            skiprows=1,
            delimiter=","
            #dtype=np.int32,
        ),
    ...
    df_temp = pd.read_csv(os.path.expanduser(path_file))
    ...
    return train_data_loader, config
