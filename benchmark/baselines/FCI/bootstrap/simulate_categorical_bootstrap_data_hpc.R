# ==============================================================================
# File        : simulate_categorical_bootstrap_data_hpc.R
...
rep_idx <- as.numeric(args[4])       # from selected_replicas.txt
b <- as.numeric(args[5])            # bootstrap ID
path_input <- args[6]               # output base path

run_bootstrap_replication <- function(rep_idx, b, name, m_id, sample_sizes, path_input) {
  ...
  for (n in sample_sizes) {
    ...
    # Output path
    out_dir <- file.path(path_input, name, as.character(n))
    dir.create(out_dir, recursive = TRUE, showWarnings = FALSE)
    
    # Write datasets
    write.csv(data_boot, file.path(out_dir, sprintf("input_0L_%d_%d.csv", rep_id, b)), row.names = FALSE)
    write.csv(data_boot[, colnames(pag_10L)],
              file.path(out_dir, sprintf("input_10L_%d_%d.csv", rep_id, b)), row.names = FALSE)
    write.csv(data_boot[, colnames(pag_20L)],
              file.path(out_dir, sprintf("input_20L_%d_%d.csv", rep_id, b)), row.names = FALSE)
  }
}

# Execute
run_bootstrap_replication(rep_idx, b, name, m_id, sample_sizes, path_input)
