...
for (name in names) {
  ...
  for (sample_suffix in names(sample_groups)) {
    ...
    script_content <- c(
      ...
      "Rscript /mnt/beegfs/home/nlagrang/stagein/nlagrang/simulate_categorical_bootstrap_data_hpc.R \\",
      "  \"$NAME\" \"$M_ID\" \"$SAMPLE_SIZES\" \"$REPLICA_ID\" \"$B\" \"$SCRATCH_DIR\"",
      "",
      "# 2. Run FCI inference",
      "conda activate conda_envs/causal_env",
      "python /mnt/beegfs/home/nlagrang/stagein/nlagrang/run_fci_bootstrap.py \\",
