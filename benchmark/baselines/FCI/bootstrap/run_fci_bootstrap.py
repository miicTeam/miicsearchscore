...
def get_fci_params(name: str):
    name = name.lower()
    if name == "alarm":
        return dict(alpha=0.05)
    elif name == "insurance":
        return dict(alpha=0.005, depth=2, max_path_length=1)
    elif name == "barley":
        return dict(alpha=0.0005, depth=1, max_path_length=0)
    elif name == "mildew":
        return dict(alpha=0.0005, depth=1, max_path_length=0)  

def adj_fci(file: str, fci_params: dict) -> pd.DataFrame:
    """
    Infer the PAG using the FCI algorithm with the G-squared test.

    Args:
        file (str): CSV file path
        fci_params (dict): parameters for the FCI algorithm
    ...
    matrix_pag = g.graph
    pag = np.zeros((len(matrix_pag), len(matrix_pag)))
    nm = list(data.columns)
    pag_df = pd.DataFrame(matrix_pag, index=nm, columns=nm)
    return pag_df

def main():
    parser = argparse.ArgumentParser(description="Run FCI on bootstrap data")
    ...
    parser.add_argument("--b", type=int, required=True, help="Bootstrap ID")
    ...
    latents = ["0L", "10L", "20L"]
    fci_params = get_fci_params(args.name)

    for n in sample_sizes:
        for latent in latents:
            filename = f"input_{latent}_{args.rep_idx}_{args.b}.csv"
            ...
            output_file = f"adj_FCI_{latent}_{args.rep_idx}_{args.b}.csv"
            ...
            print(f"Running FCI on {input_path} with params: {fci_params}")
            pag = adj_fci(input_path, fci_params)
